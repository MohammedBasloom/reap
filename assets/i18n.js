...
(function () {
  ...
  const D = {
    ...
    "— whatever remains after covering the month's own costs — is retained to fund upcoming periods, sweeps the loan (interest first, then principal), and is distributed to equity only once no further contributions are needed. Any residual balance at the project's natural exit month is force-cleared from exit proceeds.": "— ما يتبقى بعد تغطية تكاليف الشهر نفسه — يُحتجز لتمويل الفترات القادمة، ويُسدِّد القرض (الفوائد أولًا ثم أصل الدين)، ولا يُوزَّع على الملكية إلا بعد انتفاء الحاجة لمساهمات إضافية. وأي رصيد متبقٍ عند شهر التخارج الطبيعي يُسدَّد من عوائد التخارج.",
